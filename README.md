...
An always-on home server and remote dev box built on a rooted **NVIDIA SHIELD TV Pro (2015, "foster")** running **LineageOS 22.2 / Android 15** without reflashing the kernel. This repo documents the device, how it is rooted and flashed, the Docker stack it runs, and how it pulls its own updates.

Every hardware and OS figure below is read directly from the device over network ADB (`10.0.0.88:5555`). Manufacturer specs that cannot be read over ADB are marked **_(spec)_**. Personal identifiers (serial, full MAC, public IPv6) are redacted; the LAN IP `10.0.0.88` is kept on purpose.
...
- The device exposes only the 4× A57 cluster; nothing past `cpu3` is presented to the OS and all cores report part `0xd07`.
...
The 4.9 kernel (and its broken Tegra bridge/veth path) is the single biggest constraint shaping the Docker work below.

Rooting and flash reference: **[`docs/01-rooting-and-lineageos.md`](docs/01-rooting-and-lineageos.md)**.
...
| [`tools/`](tools/) | Host-side tooling (the app-sideload updater). |
...
- An Android `init` service, `shield-deploy` (`/system/etc/init/shield-deploy.rc`), runs on boot and on-demand. It is a `oneshot` service: it runs the deploy pass and exits, so `init.svc.shield_deploy=stopped` is the correct state after a successful run.
- The service runs `deploy/pull-and-deploy.sh`, which waits for the dockerd socket, then runs `git pull` inside a throwaway `alpine/git` container (`deploy/git-sync.sh`) — git runs in a container because the Toybox userland has no `git` binary. The clone lives at `/data/NVidiaShieldPro`.
- When `HEAD` moves, `deploy/redeploy.sh` re-runs the idempotent launchers (each does `docker rm -f` before any port-free assert, so a re-run while a service is up does not false-fail on "port in use"). When `HEAD` is unchanged the run logs "no change" and does nothing.

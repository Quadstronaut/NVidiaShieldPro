...
- **A6 — Full Claude-state persistence + mobile UI affordances** *(added 2026-06-23, bug-fix per user request; logged deviation that refines D6/I10).* The `claude-home` volume now mounts the **whole `/home/claude`** (steering nested under `.claude/`), so **`~/.claude.json`** — onboarding flag, theme, OAuth account, per-project `hasTrustDialogAccepted` — persists across container restart/reboot instead of resetting on the ephemeral layer (it is a *sibling* of `~/.claude/`, so the old `~/.claude`-only mount never captured it). The launcher idempotently seeds only *missing* first-run flags (onboarding / theme / bypass-permissions + trust for `/data/claude` and `/data/claude/GIT/*`). Two **client-only** UI additions: a collapsible **Prompts** overlay (chips float over the terminal; the TTY keeps full height) and a **login-link banner** (detects a wrapped OAuth URL in the pty stream and surfaces Open/Copy + a paste-code box). The server contract (§3) is unchanged. **I6 still holds** — live tmux sessions are still not preserved across a restart; only Claude's config/auth state now is.

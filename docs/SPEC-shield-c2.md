...
**Launcher contract — `docker-bringup/c2.sh`** mirrors `portainer.sh` style: `BB=/data/docker/bin/busybox`, `DOCKER="/data/docker/bin/docker -H unix:///data/docker/docker.sock"`, preflight `docker version` check, build-or-load image, remove any prior `shield-c2`, then `docker run -d` with the flags+mounts+env above, port-free assertion before run, finish with a `docker ps` table. Idempotent.

...
This is the design spec for the `shield-c2` service, which is built and running on the Shield (`http://10.0.0.88:8888`). The decisions (D1–D7), invariants (I1–I12), acceptance criteria, and tests below are the implemented contract.
...
- **A2 — NO AUTHENTICATION.** The page is unauthenticated, matching the host's existing Portainer (9000) and Uptime-Kuma (3001), for a trusted home LAN. No login/token/session/CSRF-token machinery (`SHIELD_C2_TOKEN`, `/login`, `/logout`, `/api/csrf`, session cookies). **KEEP** the docker-socket allowlist (I2) — it is the real blast-radius limiter and is independent of auth. Mutations stay **POST-only** (not triggerable by a stray GET/img tag). The threat-model doc states honestly that the surface is unauthenticated by choice and anyone on the LAN can drive containers.
...
- Networking: bridge/veth is **broken** on this kernel (ARP INCOMPLETE across docker0). **MANDATORY `--network host`.** No `-p`, no bridge nets. Container binds a host port directly on 10.0.0.88. Occupied host ports: Portainer 9000/9443/8000, Uptime-Kuma 3001.
- SELinux permissive, root available. To see HOST cpu/mem/net/thermal the container bind-mounts host `/proc`, `/sys`, `/data` **read-only** and reads from the mount (never the container's own `/proc`).
...
- **D4 — Host port: 8888** (per A1; env-overridable). Free vs Portainer/Kuma. Launcher asserts the port is free before binding.
...
**Launcher contract — `docker-bringup/c2.sh`** mirrors `portainer.sh` style: `BB=/data/docker/bin/busybox`, `DOCKER="/data/docker/bin/docker -H unix:///data/docker/docker.sock"`, preflight `docker version` check, build-or-load image, remove any prior `shield-c2`, port-free assertion, then `docker run -d` with the flags+mounts+env above, finish with a `docker ps` table. Idempotent.
...
- **I10 RESOURCE FRUGALITY:** image ≤250 MB (hard ceiling 350 MB); steady-state RSS target <150 MB so it coexists with Portainer/Kuma/workloads on 3 GB. No bundled smartctl/heavy deps.
- **I11 SOCKET PATH FIDELITY:** host socket `unix:///data/docker/docker.sock` mapped to where the app expects it; launcher and app agree (Portainer convention: → `/var/run/docker.sock`).
...
- **AC8 C2 LIST:** `GET /api/containers` lists the running stack (portainer, uptime-kuma, …) with id/name/image/state.
...
- **AC17 PORT JUSTIFIED & FREE:** 8888 documented as unused vs Portainer (9000/9443/8000) and Kuma (3001); launcher asserts free; `SHIELD_C2_PORT` makes a collision a one-line change (D4).
...
# T12 git preflight (AC16/I12)
git rev-parse --is-inside-work-tree
grep -qE 'node_modules|\.svelte-kit|/build|\*\.apk|\*\.zip|\*\.img|\*\.tgz' .gitignore
...
A pass is every command exiting 0, with the corresponding evidence retained.
...
- `docker-bringup/c2.sh` — launcher mirroring `portainer.sh` conventions (host net, `--restart=always`, ro `/proc`/`/sys`/`/data`, rw socket, env, idempotent, port-free assertion).
- `docs/THREAT-MODEL.md` — the honest unauthenticated-by-choice threat model.

## 8. Git preflight

`git init -b master` in the repo root (default branch `master`), commit the `.gitignore` (excludes node_modules/build/.svelte-kit and the existing `.apk`/`.apkm`/`.img`/`.zip`/`.tgz` blobs + `docker-bringup/docker/`), and an initial commit of the existing tree. No large blob or secret enters history (I12).
